...
describe("startGame", () => {
  it("starts the Game, locking in the Scoring Mode and moving the Room to Active", () => {
    ...
    const result = startGame(room, { type: "StartGame", roomCode: "ABCD", scoringMode: "Traditional" });
  ...
  it("rejects starting with more than 8 Players, leaving the Room state unchanged", () => {
    ...
    const result = startGame(room, { type: "StartGame", roomCode: "ABCD", scoringMode: "Rascal" });
  ...
  it("accepts the boundary Player counts of 3 and 8", () => {
    expect(
      startGame(lobbyWith(3), { type: "StartGame", roomCode: "ABCD", scoringMode: "Traditional" })
        .state?.status,
    ).toBe("Active");
    expect(
      startGame(lobbyWith(8), { type: "StartGame", roomCode: "ABCD", scoringMode: "Traditional" })
        .state?.status,
    ).toBe("Active");
  });
  ...
  it("rejects starting a Room Code with no matching Room", () => {
    const result = startGame(null, { type: "StartGame", roomCode: "ZZZZ", scoringMode: "Traditional" });
